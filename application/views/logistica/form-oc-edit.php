					<div class="col-12 iq-card my-3">
						<div class="iq-card-header d-flex justify-content-between">
							<div class="iq-header-title"><h4>Editar Orden de Compra</h4></div>
						</div>
						<div class="iq-card-body">
							...
							<form method="post" id="form_empresa" action="<?=base_url()?>logistica/regoc" enctype="multipart/form-data"
									class="needs-validation form-horizontal" novalidate="">
								<input type="hidden" name="tiporegistro" value="editar" />
								<input type="hidden" id="tabla" name="tabla" value="orden_compra_detalle" />
								<input type="hidden" id="idorden" name="idorden" value="<?=$orden->idorden?>" />
									...
									<div class="col-12 my-1">
										<div class="row">
											...
											<div class="col-md-2 col-lg-2">
												<div class="row">
													...
													<?
														foreach($anio as $row):	?>
															<option value="<?=$row->anio?>" <?=$row->anio===$orden->anio? 'selected':''?>><?=$row->anio?></option>
													<?	endforeach;	?>
													</select>
												</div>
											</div>
										...
										<div class="row mt-3">
											...
											<div class="col-md-4 col-lg-4">
												<div class="row">
													<input type="date" name="fecha" id="fecha" value="<?=$orden->fecha?>" class="form-control form-control-sm">
												</div>
											</div>
										</div>
										<div class="row mt-3">
											...
											<div class="col-md-4 col-lg-4">
												<div class="row">
													...
													<?
														foreach($emp as $row):	?>
															<option value="<?=$row->idempresa;?>" <?=$row->idempresa===$orden->idempresa? 'selected':'';?>>
												...
												<div class="row">
													...
													<?
														foreach($centro as $row):	?>
															<option value="<?=$row->idcentro;?>" <?=$row->idcentro===$orden->idcentro? 'selected':'';?>>
																<?=$row->centro_costos;?></option>
													<?	endforeach;	?>
													</select>
													<label class="invalid-feedback">Campo requerido</label>
												</div>
										...
										<div class="row mt-3">
											...
											<div class="col-md-6 col-lg-4">
												<div class="row">
													<input type="hidden" id="idproveedor" name="idproveedor" value="<?=$orden->idproveedor?>" />
													<input type="text" class="form-control form-control-sm mayusc col-10 disabled" id="proveedor"
														value="<?=$prov->nombre_comercial?>" required />
										...
										<div class="row mt-3">
											...
											<div class="col-md-2 col-lg-2">
												<div class="row">
													...
													<?
														foreach($tipo as $row):	?>
															<option value="<?=$row->idtipopago?>" <?=$row->idtipopago===$orden->idtipopago? 'selected':'';?>>
																<?=$row->tipo_pago?></option>
													<?	endforeach;	?>
													</select>
												</div>
											</div>
												...
												<div class="row">
													...
													<?
														foreach($medio as $row):	?>
															<option value="<?=$row->idmediopago?>" <?=$row->idmediopago===$orden->idmediopago? 'selected':'';?>>
																<?=$row->medio_pago?></option>
													<?	endforeach;	?>
													</select>
												</div>
											</div>
										...
										<div class="row mt-3">
											...
											<div class="col-md-2 col-lg-2">
												<div class="row">
													...
													<?
														foreach($mon as $row):	?>
															<option value="<?=$row->idtipomoneda?>" <?=$row->idtipomoneda===$orden->idtipomoneda? 'selected':'';?>>
											...
											<div class="col-md-2 col-lg-2">
												<div class="row">
													<input type="text" class="form-control form-control-sm moneda" name="tcambio"
														value="<?=number_format($orden->tipo_cambio,'2','.',',')?>" id="tcambio" />
												</div>
											</div>
										</div>
										<div class="row mt-3">
											...
											<div class="col-md-1 col-lg-1">
												<div class="row">
													<input type="text" class="form-control form-control-sm moneda" name="importe" 
														value="<?=number_format($orden->importe,'2','.',',')?>" id="importe" />
												</div>
											</div>
											<label class="control-label col-md-2 col-lg-2 align-self-center mb-0" for="impuesto">Impuesto:</label>
											<div class="col-md-1 col-lg-1">
												<div class="row">
													<input type="text" class="form-control form-control-sm moneda" name="impuesto" 
														value="<?=number_format($orden->impuesto,'2','.',',')?>" id="impuesto" />
												</div>
											</div>
											<label class="control-label col-md-2 col-lg-2 align-self-center mb-0" for="total">Total:</label>
											<div class="col-md-1 col-lg-1">
												<div class="row">
													<input type="text" class="form-control form-control-sm moneda" name="total" 
														value="<?=number_format($orden->total,'2','.',',')?>" id="total" />
												</div>
											</div>
										</div>
										<div class="row mt-3">
