					<div class="col-12 iq-card my-3">
						<div class="iq-card-header d-flex justify-content-between">
							<div class="iq-header-title"><h4>Editar Ingresos</h4></div>
						</div>
						...
						<form method="post" id="form_empresa" action="<?=base_url()?>logistica/regentrada" enctype="multipart/form-data"
								class="needs-validation form-horizontal" novalidate="">
							<input type="hidden" name="tiporegistro" value="editar" />
							<input type="hidden" id="tabla" name="tabla" value="guia_ingreso_detalle" />
							...
							<div class="form-row">
								<div class="col-12 my-1">
											...
											<div class="row">
												...
												<?
													foreach($estab as $row):	?>
														<option value="<?=$row->idempresa;?>" <?=$row->idempresa===$emp->idempresa? 'selected':'';?>>
									...
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												...
												<?
													foreach($alm as $row):	?>
													<option value="<?=$row->idalmacen;?>" <?=$row->idalmacen===$guia->idalmacen? 'selected':'';?>>
														<?=$row->nombre_almacen;?></option>
												<?	endforeach;	?>
												</select>
									...
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												...
												<?
													foreach($mov as $row):	?>
													<option value="<?=$row->idtipomovimiento;?>" <?=$row->idtipomovimiento===$guia->idtipomovimiento? 'selected':'';?>>
														<?=$row->tipo_movimiento;?></option>
												<?	endforeach;	?>
												</select>
												<label class="invalid-feedback">Campo requerido</label>
											</div>
									...
									<div class="row mt-3">
											...
											<div class="col-md-6 col-lg-4">
												<div class="row">
													<input type="hidden" id="idproveedor" name="idproveedor" value="<?=$guia->idproveedor?>" />
													<input type="text" class="form-control form-control-sm mayusc col-10 disabled" 
														value="<?=$prov->nombre_comercial?>" id="proveedor" required />
									...
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												...
												<?
													foreach($comp as $row):	?>
													<option value="<?=$row->idtipocomprobante;?>" <?=$row->idtipocomprobante===$guia->idtipocomprobante? 'selected':'';?>>
														<?=$row->tipo_comprobante;?></option>
												<?	endforeach;	?>
												</select>
												<label class="invalid-feedback">Campo requerido</label>
											</div>
									...
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												<input type="text" class="form-control form-control-sm mayusc" name="serie" 
													value="<?=$guia->serie_comprobante?>" placeholder="Serie Comprobante" />
											</div>
										</div>
									</div>
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												<input type="text" class="form-control form-control-sm mayusc" name="nrocomp" 
													value="<?=$guia->numero_comprobante?>" placeholder="Nro. Comprobante" />
											</div>
										</div>
									</div>
									<div class="row mt-3">
										...
										<div class="col-md-6 col-lg-4">
											<div class="row">
												<input type="text" class="form-control form-control-sm mayusc" name="obs"
													value="<?=$guia->observaciones?>" placeholder="Observaciones" />
											</div>
										</div>
									</div>
