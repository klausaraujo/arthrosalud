...
class Main extends CI_Controller
{
	...
	{
		$path =  $_SERVER['DOCUMENT_ROOT'].'/arthrosalud/public/adjuntos/anexos_locadores/'; $status = 500; $nombre = '';
	...
	{
		$path =  $_SERVER['DOCUMENT_ROOT'].'/arthrosalud/public/adjuntos/anexos_postulantes/';
		$filen = basename($this->input->get('file')); $type = '';
		if(is_file($path.$filen)){
			$size = filesize($path.$filen);
	...
	public function ver()
	{
		...
		$img = file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/arthrosalud/public/images/logo-white.png');
